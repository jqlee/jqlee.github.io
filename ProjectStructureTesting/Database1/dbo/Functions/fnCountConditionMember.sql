...
-- =============================================
Create FUNCTION [dbo].[fnCountConditionMember]
(	
...
(
	...
	select ConditionNumber,isNull(count(*),0) as MemberCount from (
	...
	select ConditionNumber,isNull(count(*),0) as MemberCount from (
		select distinct tc.Number as ConditionNumber, m.Id as MemberId,m.DepartmentId,g.GroupYear,m.RoleCode
		from [Condition] tc
		inner join [Target] t on t.ConditionNumber = tc.Number
