...
-- =============================================
CREATE FUNCTION [dbo].[fnSurveyMemberCount]
(	
...
/*
	select TargetNumber
	,isNull(count(MemberId),0) as MemberCount 
	...
	from (
		...
		,case r.Done when 1 then 1 else 0 end as RecordCount
		from [Condition] tc
		...
		left outer join Record r on r.SurveyNumber = tc.SurveyNumber and r.MatchKey = t.MatchKey and r.MemberId = m.Id
		where tc.SurveyNumber = @surveyNumber
		 --and tc.Number = isNull(@conditionNumber, tc.Number )
		 and (tc.TargetMark = 2 or tc.TargetMark = 3)
	) x
	...
	from (
		...
		where tc.SurveyNumber = @surveyNumber
		 --and tc.Number = isNull(@conditionNumber, tc.Number )
		 and (tc.TargetMark = 1 or tc.TargetMark = 4)
	) x
	group by TargetNumber
