...
-- =============================================
CREATE PROCEDURE [dbo].[sp_AnswerLog_GetChoiceCountListByTarget] 
	-- Add the parameters for the stored procedure here
	@targetGuid uniqueidentifier = null
	--, @groupTeacherId varchar(20) = null, @groupRole varchar(20) = null
...
BEGIN
	...
	select q.QuestionNumber, q.SubsetNumber, q.GroupingNumber, q.Title,q.AreaTitle
	, c.Number as ChoiceNumber, c.Text as ChoiceText, cs.Score as ChoiceScore 
	, count(rqs.Number) as [AnswerCount]
	from RecordTarget rt 
	inner join RecordScoreIndex rsi on rsi.Number = rt.IndexNumber
	inner join ChoiceScore cs on cs.ConfigNumber = rsi.ConfigNumber
	inner join Choice c on c.Number = cs.ChoiceNumber
	inner join v_QuestionUnit q on q.QuestionNumber = c.QuestionNumber
		left outer join ViewPermission vp on vp.PublishNumber = rt.PublishNumber and vp.QuestionNumber = q.QuestionNumber and vp.SubsetNumber = q.SubsetNumber and vp.GroupingNumber = q.GroupingNumber
	left outer join RecordQuestionScore rqs on rqs.TargetNumber = rt.Number and rqs.QuestionNumber = q.QuestionNumber and rqs.SubsetNumber = q.SubsetNumber and rqs.GroupingNumber = q.GroupingNumber and rqs.SelectedChoiceNumber = c.Number

	where rt.[Guid] = @targetGuid
	and 1 = case 
		when @checkPermission = 1 and vp.QuestionNumber is null then 0 else 1 end
	group by q.QuestionNumber, q.SubsetNumber, q.GroupingNumber, q.Title,q.AreaTitle
	, c.Number, c.[Text], cs.Score, q.QuestionSort,q.SubsetSort,q.GroupingSort,c.SortOrder
	order by q.QuestionSort,q.SubsetSort,q.GroupingSort,c.SortOrder
	
	/*


	declare @configNumber int
	select @configNumber = rsi.ConfigNumber 
	from RecordTarget rt 
	inner join RecordScoreIndex rsi on rsi.Number = rt.IndexNumber
	where rt.[Guid] = @targetGuid
