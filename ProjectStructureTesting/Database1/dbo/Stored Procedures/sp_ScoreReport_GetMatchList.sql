...
-- =============================================
CREATE PROCEDURE [dbo].[sp_ScoreReport_GetMatchList]
...
from (
	-- 如果依照條件查，就只看整份 (人員滿意度)
	select mc.SurveyNumber, null as GroupId, null as GroupTeacherId, null as GroupRole
		,  mc.MemberDepartmentId, mc.MemberGrade, mc.MemberRole
		, mc.MemberCount
		, count(x.RecordNumber) as RecordCount
	from (
...
exec sp_ScoreReport_GetListForTextAnswer 'dd88b465-2774-4a69-b9f5-323ef41a7e6f'

select man_grp from ican5.dbo.man
select cour_grp from ican5.dbo.course
